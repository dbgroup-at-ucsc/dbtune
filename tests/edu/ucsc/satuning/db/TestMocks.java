...
public class TestMocks {
    ...
    public static PGIndex makePGIndex(){
        class PI extends PGIndex {
            PI() {
                super(makePGIndexSchema(), 1, 0.0, 0.0, "");
            }
        }
        return new PI();  
    }
    ...
    private static PGIndexSchema makePGIndexSchema(){
        try {
            ...
            final List<DatabaseIndexColumn> columns      = Util.newList();
            final List<Boolean>             isDescending = Util.newList();
            return c.newInstance(123456, true, columns, isDescending);
        } catch (Exception e){
            throw new RuntimeException(e);
        }
    }
