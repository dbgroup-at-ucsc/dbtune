...
import edu.ucsc.satuning.spi.Parameters;
...
import static edu.ucsc.satuning.db.TestMocks.makeDB2Index;
import static edu.ucsc.satuning.db.TestMocks.makeMockPreparedStatement;
import static edu.ucsc.satuning.db.TestMocks.makeMockPreparedStatementSwitchOffOne;
import static edu.ucsc.satuning.db.TestMocks.makeMockStatement;
import static edu.ucsc.satuning.db.TestMocks.makeMockStatementSwitchOffOne;
import static edu.ucsc.satuning.db.TestMocks.makePGIndex;
import static edu.ucsc.satuning.db.TestMocks.makeResultSet;
import static edu.ucsc.satuning.util.Util.str;
...
public class DatabasePackageTest {
    ...
    private static void checkRecommendIndexesFromSQL(DatabaseConnection<?>... connections) throws Exception {
        for(DatabaseConnection<?> each : connections){
            ...
            assertTrue("3 indexes", Util.count(found) == 3);
        }
    }
    ...
    public void testFixCandidatesScenario() throws Exception {
        final List<DB2Index> db2CandidateSet = Util.newList();
        db2CandidateSet.add(makeDB2Index());
        final List<PGIndex>  pgCandidateSet  = Util.newList();
