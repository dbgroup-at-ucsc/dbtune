...
public class IndexPartitions {
	...
	public IndexPartitions(Snapshot snapshot, BitSet[] partitionBitSets) {
		...
		for (BitSet bs : partitionBitSets) {
			...
			for (int i = bs.nextSetBit(0); i >= 0; i = bs.nextSetBit(i+1)) {
				DB2Index idx = snapshot.findIndexId(i);
				if (idx != null) {
					if (subset != null)
	...
	public final void merge(DB2Index i1, DB2Index i2) {
	...
	public class Subset implements Iterable<DB2Index> {
		private final int sumIndexIds;
		private final int minIndexIds;
		
		private int[] indexIds;
		
		private java.util.TreeMap<Integer,DB2Index> map = new java.util.TreeMap<Integer,DB2Index>();
		
		Subset(DB2Index index) {
		...
		Subset(Subset s1, Subset s2) {
			...
			{ int i = 0; for (DB2Index idx : map.values()) indexIds[i++] = idx.internalId(); }
		...
		public BitSet bitSet() {
			...
			for (DB2Index x : this) bs.set(x.internalId());
			return bs;
		}
	...
	private class SubsetList {
		...
		final int whichSubset(DB2Index index) {
