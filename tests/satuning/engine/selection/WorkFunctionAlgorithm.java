...
import satuning.engine.selection.IndexPartitions.Subset;
...
public class WorkFunctionAlgorithm {
	...
	public void vote(DB2Index index, boolean isPositive) {
	...
	public List<DB2Index> getRecommendation() {
		ArrayList<DB2Index> rec = new ArrayList<DB2Index>(Configuration.maxHotSetSize);
		for (SubMachine subm : submachines) {
			for (DB2Index index : subm.subset)
	...
	public void repartition(IndexPartitions newPartitions) {
		...
		for (int newSubsetNum = 0; newSubsetNum < newSubsetCount; newSubsetNum++) {
			...
			for (int stateNum = 0; stateNum < newSubmachine.numStates; stateNum++) {
				...
				i = 0;
				for (DB2Index index : newSubmachine.subset) {
					int mask = (1 << (i++));
	...
	private boolean isRecommended(DB2Index idx) {
...
//	}
	...
	private static class SubMachine implements Iterable<DB2Index> {
...
//			}
		...
		public void vote(TotalWorkValues wf, DB2Index index, boolean isPositive) {
