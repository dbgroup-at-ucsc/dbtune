...
import edu.ucsc.satuning.engine.selection.WorkFunctionAlgorithm.TotalWorkValues;
...
//	}
	...
	public BitSet[] optimalSchedule(IndexPartitions<I> parts, int queryCount, Iterable<ProfiledQuery<I>> qinfos) {
		...
		for (int subsetNum = 0; subsetNum < parts.subsetCount(); subsetNum++) {
			IndexPartitions<I>.Subset subset = parts.get(subsetNum);
