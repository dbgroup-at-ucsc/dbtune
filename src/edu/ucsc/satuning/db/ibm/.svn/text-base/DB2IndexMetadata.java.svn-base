...
import edu.ucsc.satuning.db.ibm.DB2IndexSchema.UniqueRule;
import edu.ucsc.satuning.db.ibm.DB2IndexSchema.ReverseScanOption;
import edu.ucsc.satuning.db.ibm.DB2IndexSchema.TypeOption;
import edu.ucsc.satuning.util.BitSet;
import edu.ucsc.satuning.util.ToDoException;
...
public class DB2IndexMetadata implements Serializable {
    ...
    public double creationCost(DatabaseIndexExtractor<DB2Index> indexExtractor, DatabaseWhatIfOptimizer<DB2Index> whatIfOptimizer) throws SQLException{
        ...
        indexExtractor.fixCandidates(Util.<DB2Index>newLinkedList());
        return whatIfOptimizer.whatIfOptimize(sql.toString())
                            .using(new BitSet(), new BitSet())
                            .toGetCost();
    }
	
	public double creationCost(DB2Connection conn) throws SQLException {
		int i, n;
