...
public class Strings {
   ...
   * <code>null</code>. Case-insensitivity is defined as by {@link String#equalsIgnoreCase(String)}.
   *
  ...
  public static String join(Iterable<?> objects, String delimiter) {
    ...
    while (i.hasNext()) { result.append(delimiter).append(i.next()); }
  ...
  public static String[] objectsToStrings(Object[] objects) {
    ...
    for (Object o : objects) { result[i++] = o.toString(); }
  ...
  public static void formatIdentifier(String str, StringBuilder sbuf) {
    ...
    if (!Character.isLetter(str.charAt(0))) { simple = false; } else {
      simple = true;
    ...
    if (simple) { sbuf.append(str); } else {
      sbuf.append('"');
      for (int i = 0; i < strlen; i++) {
        char c = str.charAt(i);
        sbuf.append(c);
  ...
  public static void formatStringLiteral(String str, StringBuilder sbuf) {
    ...
    for (int i = 0; i < strlen; i++) {
      char c = str.charAt(i);
      if (c == '\'') { sbuf.append("''"); } else { sbuf.append(c); }
    }
    sbuf.append('\'');
  }
}
