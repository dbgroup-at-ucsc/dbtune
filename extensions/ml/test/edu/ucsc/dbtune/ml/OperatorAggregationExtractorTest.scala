...
class OperatorAggregationExtractorTest {
  ...
  @Test def testBasicUsage = {
    ...
    assertEquals(155.0+428.0, instances.instance(0).value(0), 0.1)
    ...
    assertEquals(926.37,      instances.instance(0).value(4), 0.1)
    ...
    assertEquals(115.11,  instances.instance(1).value(2), 0.1)
    ...
    assertEquals(12.2,    instances.instance(1).value(6), 0.1)
...
object OperatorAggregationExtractorTest {
    val plan1 = {
        ...
        plan.setChild(root, new Operator("Seq Scan",155.0,10000), StatementPlan.LEFT)
        plan.setChild(root, new Operator("Seq Scan",428.0,10000), StatementPlan.RIGHT)
        plan
    }
    val plan2 = {
        ...
        plan.setChild(root, new Operator("Index Scan",12.2,1234567), StatementPlan.LEFT)
        plan
    }
}
