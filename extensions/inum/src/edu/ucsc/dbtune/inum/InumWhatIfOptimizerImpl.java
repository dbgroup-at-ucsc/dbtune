...
public class InumWhatIfOptimizerImpl implements InumWhatIfOptimizer {
   ...
   *
   * @param catalog a container of schema objects.
   * @param connection a live database connection to postgres
   */
   ...
   * construct a new {@code InumWhatIfOptimizer} object.
   *
   * @param inum a new instance of INUM.
   */
  ...
  /**
   * shut down INUM. This method may be called when the main component of the dbtune API request a
   * closing of their {@link Connection} object. Once this db connection is closed, there is not
   * need to keep INUM 'on' too.
   */
  public void endInum() {
  ...
  @Override public double estimateCost(String query) throws SQLException {
    if (getInum().isEnded()) { startInum(); }
    return estimateCost(query, EMPTY_CONFIGURATION);
  }

  @Override public double estimateCost(String query, Set<Index> hypotheticalIndexes)
      throws SQLException {
    if (getInum().isEnded()) { startInum(); }
