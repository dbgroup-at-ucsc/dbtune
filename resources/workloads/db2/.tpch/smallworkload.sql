...
--query 05
--  error Can't determine object associated to leaf node: 17: INDEX.SCAN(cost=202.3952178955078 rows=150000)
select
	n_name,
	sum(l_extendedprice * (1 - l_discount)) as revenue
from
...
--query 20
-- error, sql: [ category=SELECT text=" SELECT L_QUANTITY , L_SHIPDATE , L_SUPPKEY , L_PARTKEY FROM TPCH.LINEITEM WHERE (L_SHIPDATE < '1997-01-01') AND 
-- ('1996-01-01' <= L_SHIPDATE) AND (L_SUPPKEY = PS_SUPPKEY) AND (L_PARTKEY = PS_PARTKEY)"]
select
	s_name,
	s_address
from
